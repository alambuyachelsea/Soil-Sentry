...
def update_soil_moisture_and_test_pump():
    while True:
        for plant in plants:
            plant.get_current_water_level()
            # Test mini pump water if necessary
            if plant.get_current_water_level() < plant.water_needs:
                print(f"Pumping water for {plant.name}")
                plant.pump_water()
                time.sleep(5)  # Simulate pumping water for 5 seconds
                plant.stop_pump()
                print(f"Water pumped for {plant.name}")
        time.sleep(10)  # Adjust the interval as needed

# Start updating soil moisture levels and testing mini pump water
update_soil_moisture_and_test_pump()